...
for filePath in filePaths:
    ...
    addingToAreaData = False

    for line in lines[li:]:
        line = line.replace("thisAreaSaveData.", "activeSave->areaData.areaData" + name + ".");
        if line.startswith("void onUpdate"):
            ...
            onLeave = "\n"
            continue
        elif line.startswith("struct AreaData"):
            addingToAreaData = True
            areaData = "\n"
            continue
    ...
    if len(areaData) > 0:
        areaSaveDataHpp += "struct AreaData" + name + "{" + areaData + "};\n"
        areaDataStructs += "    AreaData" + name + " areaData" + name + ";\n"
...
file = open(genPath+"areas.hpp", "w")
file.write(areasHpp)
file.close()

file = open(genPath+"areas.cpp", "w")
file.write(areasCpp)
file.close()

file = open(genPath+"areaSaveData.hpp", "w")
file.write(areaSaveDataHpp)
file.close()
